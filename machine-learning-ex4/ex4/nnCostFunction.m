...
%
for exampleCounter = 1 : m
  Z_of_2 = Theta1 * X(exampleCounter, :)';
  ...
  result = labels == y(exampleCounter, :);

 for classCounter = 1 : num_labels
     yK = result(classCounter, :);
     h_of_X =  a_of_3;
     Log_of_h_of_x = log(h_of_X);
     kLog_of_h_of_x = Log_of_h_of_x(classCounter);     
     
     J =  J + (((-1) * yK * kLog_of_h_of_x) - ((1-yK) * log(1 - h_of_X(classCounter))));

end
end
% -------------------------------------------------------------

sq_theta1 = sum(sumsq(Theta1(:,2:input_layer_size+1)));
sq_theta2 = sum(sumsq(Theta2(:,2:hidden_layer_size+1)));


   J = ((J / m) + ( (lambda / (2 * m)) *sum(sq_theta1 + sq_theta2)));
